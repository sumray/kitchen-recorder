...
public class IdGenerator {
	...
	public static final String DATACENTER_ID_PROP = "qiyuesuo.datacenter.id";
	...
	public void modifyWorkId(long newWorkId) {
		if (newWorkId > maxWorkerId || newWorkId < 0) {
			throw new IllegalArgumentException(	String.format("worker Id can't be greater than %d or less than 0", maxWorkerId));
		}
		if(this.workerId != newWorkId) {
			this.workerId = newWorkId;
		}
	}

	public void modifyDatacenterId(long newDatacenterId) {
		if (newDatacenterId > maxDatacenterId || newDatacenterId < 0) {
			throw new IllegalArgumentException(	String.format("datacenter Id can't be greater than %d or less than 0", maxDatacenterId));
		}
		if(this.datacenterId != newDatacenterId) {
			this.datacenterId = newDatacenterId;
		}
	}

	public static void main(String[] args) {
		IdGenerator a = new IdGenerator(1L, 1L);
